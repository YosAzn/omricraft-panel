...
export default function GlobalRepository({ allAddons, customAddons, onAdd, onDelete, t, lang, userRole }) {
  ...
  return (
    <div className="animate-in fade-in duration-300">
        ...
        {userRole === 'admin' && (
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all border ${showModpackForm ? 'bg-pink-600 text-white border-pink-500' : 'bg-zinc-900 border-pink-500/40 hover:border-pink-500 text-pink-300'}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-all border ${showDatapack ? 'bg-orange-600 text-white border-orange-500' : 'bg-zinc-900 border-orange-500/40 hover:border-orange-500 text-orange-300'}`}
            >
      ...
      {/* Datapack Builder — controlled by the row's Datapack button (orange = datapacks group). */}
                 ...
                 <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800">
                   {['free', 'gemini'].map(m => (
                     <button
                       ...
                       className={`px-3 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap ${builderModel === m ? 'bg-pink-600 text-white' : 'text-zinc-400 hover:text-zinc-200'}`}
                     >
                   ...
                   <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-2 max-h-72 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                     {builderResult.mods.map(m => (
                       <div key={m.slug} className="flex items-start justify-between gap-2 p-2 rounded-md border border-transparent hover:border-zinc-800 hover:bg-zinc-900 transition-colors">
                         ...
                         <a href={m.url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-pink-400 hover:text-pink-300 flex-shrink-0 p-1" title={m.url}>
